...
public class Solution extends AbstractSolution {
    ...
    public record Antenna(char type, Position position) {
        ...
        private List<Position> getAntinodes(Position otherPosition, List<List<Character>> matrix) {
            ...
            while (true) {
                otherPosition = new Position(otherPosition.x + x, otherPosition.y + y);
                if (!isInbound(otherPosition, matrix)) {
                    break;
                }
                positions.add(otherPosition);
            }
